...
public class PlanetSystem implements Comparable<PlanetSystem> {
    ...
    public int compareTo(PlanetSystem o) {
        double totalPSMass = 0;

        for (Planet p : this.planets) {
            totalPSMass += p.getMassInKg();
        }
        double totalPSMass2 = 0;

        for (Planet p : o.planets) {
            totalPSMass2 += p.getMassInKg();
        }
        if (totalPSMass < totalPSMass2){
            return -1;
        }
        else if (totalPSMass > totalPSMass2){
            return 1;
        }
        else{
            return 0;
